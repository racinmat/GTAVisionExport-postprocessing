...
from gta_math import construct_model_matrix, create_model_rot_matrix, rot_matrix_to_euler_angles, create_rot_matrix, \
    ...
    relative_and_absolute_camera_to_car_position
...
def car_to_camera_view_data():
    return (
        (  # car_rot_data, cam_rot_data, rel_cam_rot_data
        ...
        (
            ...
            [-34.02688980102539000, 1.26241147518157960, 89.65865325927734000],
            [-30., 0., 90.]
        ),
        (
            [-1.26793634891510000, -4.02545595169067400, -0.25146627426147460],
            [-28.76274490356445300, 4.02084207534790000, 179.86648559570312000],
            [-30., 0.0, 180.]
        ),
        (
            [-1.26793634891510000, -4.02545595169067400, -0.25146627426147460],
            [-94.01548767089844000, 1.17979562282562260, 89.91169738769531000],
            [270., 0.0, 90.]
        ),
    )
...
def car_to_camera_position_data():
    return (
        (  # car_pos_data, car_rot_data, rel_cam_pos_data, cam_pos_data
...
def camera_to_car_position_data():
    return (
        (  # car_pos_data, cam_pos_data, cam_rot_data, rel_cam_pos_data, rel_cam_rot_data
            [2524.95751953125000000, 3333.01220703125000000, 52.35118103027344000],
            [2524.83105468750000000, 3334.53613281250000000, 53.38750457763672000],
            [-1.26793634891510000, -4.02545595169067400, -0.25146627426147460],
            [-0.05999999865889549, 1.50000000000000000, 1.07649993896484380],
        ...
        (
            ...
            [-34.02688980102539000, 1.26241147518157960, 89.65865325927734000],
            [15.93999958038330000, 17.50000000000000000, 6.07649993896484400],
            [-30.00000000000000000, 0.00000000000000000, 90.00000000000000000],
        ),
        (
            [2524.95751953125000000, 3333.01220703125000000, 52.35118103027344000],
            [2524.58129882812500000, 3366.63574218750000000, 57.69403457641601600],
            [-28.76274490356445300, 4.02084207534790000, 179.86648559570312000],
            [-0.05999999865889549, 33.50000000000000000, 6.07649993896484400],
            [-30.00000000000000000, 0.00000000000000000, 180.00000000000000000],
        ),
        (
            [2524.95751953125000000, 3333.01220703125000000, 52.35118103027344000],
            [2508.55883789062500000, 3350.65502929687500000, 56.91956329345703000],
            [-25.97977638244629000, -1.22395157814025880, -90.18311309814453000],
            [-16.05999946594238300, 17.50000000000000000, 6.07649993896484400],
            [-30.00000000000000000, 0.00000000000000000, 270.00000000000000000],
        ),
        (
            [2524.95751953125000000, 3333.01220703125000000, 52.35118103027344000],
            [2525.36669921875000000, 3334.53466796875000000, 53.43165969848633000],
            [-1.21343827247619630, -4.01760005950927700, -0.06144140288233757],
            [0.48000001907348633, 1.50000000000000000, 1.07649993896484380],
            [0.00000000000000000, 0.00000000000000000, 0.00000000000000000],
        ),
        (
            ...
            [-94.01548767089844000, 1.17979562282562260, 89.91169738769531000],
            [-0.05999999865889549, 17.50000000000000000, 25.07649993896484400],
            [270.00000000000000000, 0.00000000000000000, 90.00000000000000000],
        ),
    )
...
class TestGtaMath(unittest.TestCase):
    ...
    def test_car_to_camera_view_rotation(self, car_rot_data, cam_rot_data, rel_cam_rot_data):
        ...
        self.assertTrue(np.allclose(cam_rot_m, orig_cam_rot_m, atol=5.e-3))
    ...
    def test_car_to_camera_position(self, car_pos_data, car_rot_data, rel_cam_pos_data, cam_pos_data):
        ...
        self.assertTrue(np.allclose(cam_pos, orig_cam_pos, atol=7.e-2))

    @data_provider(camera_to_car_position_data)
    def test_camera_to_car_position(self, car_pos_data, cam_pos_data, cam_rot_data, rel_cam_pos_data, rel_cam_rot_data):
        cam_pos = np.array(cam_pos_data)
        cam_rot = np.array(cam_rot_data)
        cam_rel_pos = np.array(rel_cam_pos_data)
        cam_rel_rot = np.array(rel_cam_rot_data)
        orig_car_pos = np.array(car_pos_data)
        car_pos = relative_and_absolute_camera_to_car_position(cam_pos, cam_rot, cam_rel_pos, cam_rel_rot)
        self.assertTrue(np.allclose(car_pos, orig_car_pos, atol=1.e-2))
